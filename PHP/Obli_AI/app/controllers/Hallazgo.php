...
class ControllerHallazgo extends AppController{
	...
	public function listado(){
        ...
        require_once (MODELS.'Hallazgo.class.php');
       	...
       	if(!empty($proyecto_id)){
            ...
            $ciudad = $P->get('ciudad');
            $condsT = array('proyecto_id'=>$proyecto_id);
       	}
        ...
        if(!empty($categoria_id)){
            ...
            $condsT = array('categoria_id'=>$categoria_id);
        }   
        $Q = new Query;
        $Q->set('conditions',$condsT);
        $H = new Hallazgo;
        $rows = $H->fetch($Q);
        ...
        //$breadcrumb = array('Inicio'=>'?controller=index&action=index',$cat_name=>"?controller=hallazgo&action=listado&categoria=$categoria_id",$sub_name=>null);
         if(!empty($categoria_id)){
            $breadcrumb = array('Inicio'=>'?controller=index&action=index',$cat_name=>null);
             ...
             $breadcrumb = array('Inicio'=>'?controller=index&action=index','Ciudades'=>"?controller=ciudad&action=listado",$ciu_name=>"?controller=proyecto&action=listado&ciudad=$ciudad",$sub_name=>null);
         }
         
        $this->View->assign('proyecto',$proyecto_id);     
    ...
    public function update(){        
   	...
   	public function detalle(){
            ...
            $this->View->assign('latitud',$H->get('latitud'));
            $this->View->assign('longitud',$H->get('longitud'));
            $this->View->assign('actualizado',$H->get('actualizado'));
            ...
            if(!empty($proyecto)){
                $breadcrumb = array('Inicio'=>'?controller=index&action=index','Ciudades'=>"?controller=ciudad&action=listado",$ciudad_name=>"?controller=proyecto&action=listado&ciudad=$ciudad",$proy_name=>"?controller=hallazgo&action=listado&proyecto=$proyecto_id",$H->get('descripcion')=>null);
            }else{
            ...
            $this->View->assign('proyecto_name',$proy_name);
