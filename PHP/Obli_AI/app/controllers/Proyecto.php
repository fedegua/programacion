...
class ControllerProyecto extends AppController{	
	...
	public function listado(){
        ...
        $rows = $P->fetch($Q);
        if (!empty ($solo)){
            $breadcrumb = array('Inicio'=>'?controller=index&action=index','Proyectos'=>null);
        }else{
             $breadcrumb = array('Inicio'=>'?controller=index&action=index','Ciudades'=>"?controller=ciudad&action=listado",$ciu_name=>null/*"?controller=ciudad&action=listado&id=$ciudad",$P->get('nombre')=>null)*/);           
        }
        
        $this->View->assign('breadcrumb',$breadcrumb);
         $this->View->assign('solo',$solo);
   ...
   public function detalle(){
        ...
        if(!empty($id)){
            ...
            $Q->set('conditions',$conds);
            $P->loadMe($Q);
            $ciudad_id = $P->get('ciudad');                         	
                //validar que venga una proyecto.            
            if(!empty($ciudad_id)){
                //cargo nombre ciudad
                require_once (MODELS.'Ciudad.class.php');
                $conds = array('id'=>$ciudad_id);
                $Q->set('conditions',$conds);			
                $C = new Ciudad;
                $C->loadMe($Q);
            ...
            $this->View->assign('creado',$P->get('creado'));
            $this->View->assign('actualizado',$P->get('actualizado'));
            $breadcrumb = array('Inicio'=>'?controller=index&action=index',"Proyectos"=>"?controller=proyecto&action=listado&solo=a" ,$P->get('nombre')=>null);           
            $this->View->assign('breadcrumb',$breadcrumb);            
            $this->View->assign('body','Proyecto/detalle.tpl');
            $this->View->display('front_main/main.tpl');
        }   
